...
const ManageCourseContent = () => {
  ...
  const fetchCourseContent = async () => {
    try {
      setLoading(true);
      const response = await adminAPI.getCourseContent(id);
  ...
  const handleDeleteVideo = async (moduleId, videoId) => {
    ...
    try {
      await adminAPI.deleteVideo(moduleId, videoId);
  ...
  const handleVideoUpload = async (e) => {
    ...
    try {
      ...
      await adminAPI.addVideo(selectedModuleId, {
        ...videoForm,
        videoBase64: base64
      });
        ...
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center z-[60] p-4">
          <div className="bg-white dark:bg-gray-800 rounded-xl max-w-md w-full p-6">
            ...
            <form onSubmit={handleVideoUpload} className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium mb-1">Select Video File (MP4 recommended)</label>
                <div className="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 dark:border-gray-600 border-dashed rounded-md">
                  <div className="space-y-1 text-center">
                    ...
                    <p className="text-xs text-gray-500">
                      {videoFile ? videoFile.name : 'Max 50MB for demo'}
                    </p>
                  </div>
                </div>
              </div>
