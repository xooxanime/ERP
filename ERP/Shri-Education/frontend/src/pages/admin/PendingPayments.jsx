...
export default function AdminPayments() {
  ...
  const handleAction = async (id, action) => {
    ...
    try {
      ...
      fetchPayments();
    } catch { toast.error(`Failed to ${action} payment`); }
    finally { setActionLoading(null); }
  };

  const filtered = payments.filter(p => {
    const matchSearch = !search || p.student?.name?.toLowerCase().includes(search.toLowerCase()) || p.course?.title?.toLowerCase().includes(search.toLowerCase());
              ...
              <TableBody>
                {filtered.length === 0
                  ...
                  : filtered.map(p => {
                    ...
                    return (
                      <TableRow key={p._id}>
                        <TableCell className="font-medium text-sm">{p.student?.name || '—'}</TableCell>
                        <TableCell className="text-muted-foreground text-sm">{p.course?.title || p.courseName || '—'}</TableCell>
