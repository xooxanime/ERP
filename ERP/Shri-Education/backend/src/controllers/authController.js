...
export const login = async (req, res) => {
  try {
    ...
    // If requesting teacher or parent role, check approval status
    if ((role === 'teacher' || role === 'parent') && user.role !== 'admin') {
    ...
    // Determine final role to use
    const finalRole = user.role === 'admin' ? 'admin' : (user.role === 'student' && user.approvalStatus === 'approved' ? role : user.role);
